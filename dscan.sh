...
# Debug
DEBUG=0 # Set to 1 for verbose output. # IMPROV: intialize if '-v' when launch
# DB section
...
scan_one_host ()
{
  ...
  # Scan host:port and format output into an array
  while IFS='\n' read -r line; do
...
main()
{
  ...
  for SERVER in $(cat $INVENTORY); do
      ...
      if [[ $(check_ip $SERVER) -eq 0 ]]; then
          ...
          #if [[ ${#globalarray}SCAN_RETURN_VALUE -eq 0 ]]; then # Scan host port 443 # IMPROV: provide an array of ports to iterate it instead of hardcoding
          #  [[ $DEBUG -eq 1 ]] && echo "Salida de scan: $SCAN_RETURN_VALUE"
          #else
